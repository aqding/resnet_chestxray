...
class ResNet256_6_2_1(nn.Module):
    ...
    def __init__(self, block, blocks_per_layers, output_channels=4, 
                 norm_layer=nn.BatchNorm2d, zero_init_residual=False):
        """ Input batch_sizex2x256x256x1          ->
            Conv1 batch_sizex2x256x256x8            ->
            "layer1" batch_sizex2x128x128x16        ->
            "layer2" batch_sizex2x64x64x32          ->
            "layer3" batch_sizex2x32x32x64          ->
            "layer4" batch_sizex2x16x16x128         ->
            "layer5" batch_sizex2x8x8x192           ->
            "layer6" batch_sizex2x4x4x192           ->
            average pooling batch_sizex2x2x2x192    ->
            fc layer batch_sizexoutput_channelsx1
        """
        ...
        self.conv1 = nn.Conv2d(2, self.inplanes, kernel_size=3, stride=1, 
                               padding=1, bias=False)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
...
#             pred_logits = y_logits[:, 0:4]
#             bnp_logits = y_logits[:, 4:8]
#             creatinine_logits = y_logits[:, 8:12]

#             y = self.softmax(pred_logits)

#             return y, z, pred_logits, bnp_logits, creatinine_logits
...
class ResNet256_6_2_1_small(nn.Module):
    """ A residual network 6_2_1 
    with 6 "layers", 2x2 average pooling, and 1 fully connected layer.
    """
    ...
    def __init__(self, block, blocks_per_layers, output_channels=4, 
        ...
        """ Input batch_sizex2x256x256x1          ->
            ...
            fc layer batch_sizexoutput_channelsx1
        """
        super(ResNet256_6_2_1_small, self).__init__()
...
#         self.layer6 = self._make_layer(block, 192, blocks_per_layers[5], stride=2)
        self.avgpool = nn.AvgPool2d((2, 2))
        #output_channels=5
        self.fc1 = nn.Linear(4096, output_channels)
        self.softmax = nn.Softmax(dim=1)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', 
...
#         if self.output_channels == 12:
#             y = self.softmax(y_logits[0:4])
#         else:
#             y = self.softmax(y_logits)
...
def build_resnet256_6_2_1_small(block=BasicBlock, blocks_per_layers=[2, 2, 2, 2, 2, 2], 
                      pretrained=False, pretrained_model_path=None, output_channels=4, **kwargs):
    model = ResNet256_6_2_1_small(block, blocks_per_layers, output_channels=output_channels, **kwargs)
    if pretrained:
        model = model.from_pretrained(pretrained_model_path, block, blocks_per_layers,
                                      output_channels, **kwargs)
    return model
